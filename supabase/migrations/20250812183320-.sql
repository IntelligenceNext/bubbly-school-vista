-- Create a mapping table between auth users and custom roles (public.roles)
...
-- Policies: only administrators manage; users can view their own
CREATE POLICY IF NOT EXISTS "Only administrators manage custom role assignments"
...
-- Helpful index for counting members per role
CREATE INDEX IF NOT EXISTS idx_user_custom_roles_role_id ON public.user_custom_roles(role_id);

-- Ensure permission names are unique so we can upsert safely
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Seed permissions to match the provided reference UI (id auto-generated)
